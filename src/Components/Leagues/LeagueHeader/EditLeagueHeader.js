...
const EditLeagueHeader = ({
  ...
  const submit = () => {
    ...
    const redirect = () => history.push(`/league/${league.league_id}`);
    editLeague(data, redirect);
  };
  if (leagueData) {
    return (
...
const mapStateToProps = state => ({
  league: state.leagues.selectedLeague,
  failed: state.leagues.getLeagueByIdFailed
});
